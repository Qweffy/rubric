...
- **SQLite is single-writer.** Open the connection in WAL mode; the CLI holds the write lock for the duration of a run and commits per case so a crashed run leaves a partial-but-consistent suite, never a half-written row. Writes are idempotent on `(suite, prompt_version, case_id, run_id)` — re-running a case replaces its row instead of duplicating it, so retries and replays are safe.
